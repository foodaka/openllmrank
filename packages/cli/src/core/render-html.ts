import type { CallRow, CitationRow, RunRow } from "./db";
...
export function renderHtmlReport(args: HtmlReportArgs): string {
  ...
  const noData = args.calls.length === 0;
...
<div class="meta">
<div><span class="label">Runs</span><span class="value">${args.runs.length}</span></div>
<div><span class="label">Samples</span><span class="value">${totalSamples}</span></div>
<div><span class="label">Providers</span><span class="value">${new Set(brandRateRows.map((r) => r.provider)).size}</span></div>
<div><span class="label">Cost</span><span class="value">${formatMoney(cost)}</span></div>
</div>
</header>
<section class="hero">
<div class="score">
<span class="label">AI visibility score</span>
...
<div class="panel">
<h2>Run history</h2>
${history.length > 1 ? historyChart : `<p class="muted">Run history appears after a second run enters the window.</p>`}
</div>
</section>
<footer>
...
function renderGapTable(rows: GapRow[], args: HtmlReportArgs, empty: string): string {
  ...
  return `<table><thead><tr><th>Provider</th><th>Prompt</th><th>Your rate</th><th>Top competitor</th><th>Gap</th></tr></thead><tbody>${rows
    .map((g) => {
...
function computeHistory(args: HtmlReportArgs): HistoryPoint[] {
  ...
  return runOrder.map((run) => {
    const calls = args.calls.filter((c) => c.run_id === run.run_id);
...
function visibilityForRun(args: HtmlReportArgs, runId: string): number {
  ...
  for (const call of args.calls.filter((c) => c.run_id === runId)) {
...
function findRecentResponse(calls: CallRow[], rates: CitationRate[], gap: GapRow): string | null {
  ...
  const matches = calls
    .filter((c) => c.prompt_id === gap.prompt_id && providerByPrompt.get(c.prompt_id) === gap.provider)
