...
export default function WizardBrandForm({ authenticated }: { authenticated: boolean }) {
  ...
  useEffect(() => {
    ...
    setHasQuestions(s.prompts.length > 0);
      ...
      {authenticated && <WebsiteAssist website={website} hasQuestions={hasQuestions} onApply={(draft) => {
        ...
        writeWizardState({
          brand: {
            ...
            website: new URL(/^https?:\/\//i.test(website.trim()) ? website.trim() : `https://${website.trim()}`).href,
            aliases: aliasInput.split(",").map(alias => alias.trim()).filter(Boolean),
          },
